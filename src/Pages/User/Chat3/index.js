...
useEffect(() => {
    ...
    <div className='sm:w-0/12 bg-purple-400 h-full md:w-2/12'>
      <p className='mt-10'>{user?.email}</p>
       {users && users.length > 0 && users.map((item,i)=>{
        return(
              <div key={item._id} className='w-full h-10 flex bg-indigo-300 m-1 px-2 justify-between items-center relative' 
              >
  ...
  <div className='fixed bottom-2 z-50 w-full ml-64 max-sm:ml-0 h-10 px-5 flex border-gray-200'>
